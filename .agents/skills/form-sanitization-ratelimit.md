---
name: form-sanitization-ratelimit
description: Application defense engineering for lead and booking forms, Zod schema validation, phone formatting, honeypot anti-spam, and IP rate limiting.
---
...
Все входящие данные на публичных эндпоинтах `/api/leads` и `/api/booking` валидируются строгой схемой. Невалидные запросы отсекаются с кодом `400 Bad Request` до передачи в бизнес-логику:
...
- **Верификация Cloudflare Turnstile**:
  * Сервер валидирует `captchaToken` через вызов `https://challenges.cloudflare.com/turnstile/v0/siteverify` с секретным ключом. При сбое проверки форма аварийно отклоняется (fail closed).
...
- **Определение IP**:
  * Не доверять произвольному `X-Forwarded-For`. Использовать доверенную цепочку прокси или сокетное соединение `req.socket.remoteAddress`.
- **Идемпотентность и повторы**: Повторные сетевые запросы клиента не должны порождать дублирующие лиды.
