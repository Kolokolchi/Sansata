---
name: interactive-canvas-ux
description: High-performance interactive canvas and viewport UX, seamless pan & zoom, virtual element tooltips via Floating UI, and zero-jank 60/120 FPS rendering.
---
...
- **Мультитач и клавиатура**:
  * Обязательна поддержка pinch-to-zoom на смартфонах и планшетах.
  * Доступность с клавиатуры: навигация стрелками и клавишами `+` / `-`.
...
- **Подсветка без React Re-render**:
  * Не использовать тяжелый глобальный стейт `hoveredApartmentId` для изменения стилей сотен полигонов.
