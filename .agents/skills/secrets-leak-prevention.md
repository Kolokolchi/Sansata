...
description: DevSecOps secrets guarding, .gitignore env isolation, post-build client bundle scanning, and credential leak prevention.
---
...
- **Правило безопасного репортинга**: Никогда не выводить и не печатать в консоль/отчеты обнаруженные реальные значения секретов. Сообщать только имя файла, тип паттерна и рекомендацию по ротации.
