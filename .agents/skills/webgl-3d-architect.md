---
name: webgl-3d-architect
description: High-performance WebGL and Three.js 3D architecture, DRACO/Meshopt GLB loading, resource disposal pipeline, and camera constraints.
---
...
- Для любых версионно-зависимых API Three.js обязательно консультироваться через Context7 MCP.
...
- **Очистка при размонтировании (Unmount)**:
  * Отмена активного `requestAnimationFrame`.
  * Удаление всех ResizeObserver и DOM event listeners.
  * Вызов `controls.dispose()`.
  * Вызов `renderer.dispose()` и удаление canvas из DOM.
  * Корректная обработка асинхронных загрузок: если загрузка завершилась после размонтирования компонента, загруженная геометрия должна быть немедленно очищена.
...
- **Приоритет FPS над декорациями**: Измерять частоту кадров и draw calls перед добавлением постпроцессинга.
